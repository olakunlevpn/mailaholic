...
begin
   ...
   szInifile := ExpandConstant('{app}\Bin\Mailaholic.ini');

   if (FileExists(szInifile) = False) then
   begin

      if RegQueryStringValue(HKLM32, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Mailaholic_is1','InstallLocation', szInifile) then
      begin
         szInifile := szInifile + 'Bin\Mailaholic.ini';
      end;

      if (FileExists(szInifile) = False) then
      begin
        // File doesn't exist in the old installation directory.
        szInifile := ExpandConstant('{win}\Mailaholic.ini');
...
begin
   ...
   begin
     ...
     Font := moreInfoFont;
 	 ...
 	 // Create key page
   g_pageAccessKey := CreateInputQueryPage(wpSelectTasks, 'Mailaholic Security', 'Specify main password','The installation program will now create a Mailaholic user with administration rights. Please enter a password below. You will need this password to be able to manage your Mailaholic installation, so please remember it.');	
...
begin
    ...
    if RegQueryStringValue(HKLM32, 'SOFTWARE\Microsoft\Windows\CurrentVersion\Uninstall\Mailaholic_is1','InstallLocation', installFolder) then
    begin
...
begin
	...
	if not IsNetFrameworkInstalled() then begin
        MsgBox('Mailaholic requires .NET Framework 4.5'#13#13
            'Please install this version and then re-run the Mailaholic setup program.', mbInformation, MB_OK);
        result := false;
		Exit;
	end;
	
	if (FindWindowByWindowName('Mailaholic Administrator') > 0) then
	begin
		MsgBox('Mailaholic Administrator is started. You must close down this application before starting the installation.',mbInformation, MB_OK);	
		Result := false;
		Exit;
	end;
	...
	if (Result = true) then
	begin
		if (FindWindowByWindowName('Mailaholic Database Setup') > 0) then
		begin
			MsgBox('Mailaholic Database Setup is started. You must close down this application before starting the installation.',mbInformation, MB_OK);	
			Result := false;
			Exit;
		end;	
	end;
	...
	if (Result = true) then
	begin
		if (FindWindowByWindowName('Mailaholic Database Upgrader') > 0) then
		begin
			MsgBox('Mailaholic Database Upgrader is started. You must close down this application before starting the installation.',mbInformation, MB_OK);	
			Result := false;
			Exit;			
		end;	
	end;	
	...
	begin
		...
		begin
			MsgBox('Mailaholic DBSetup is started. You must close down this application before starting the installation.',mbInformation, MB_OK);	
			Result := false;
			Exit;			
		end;	
	end;	
	...
	if (Result = true) then
	begin
		if (IsServiceRunning('Mailaholic') <> True) And (CheckPorts() < 0) then
		begin
			// The Mailaholic isn't running, but someone is blocking the ports.
			//
			sMessage := 'The Mailaholic Setup has detected that one or several of the TCP/IP ports 25, 110 and 143 are already in use.' + Chr(13) + Chr(10) + 'This indicates that there already is an email server running on this computer.' + Chr(13) + Chr(10) + 'If you plan to use any of these ports with Mailaholic, the already existing server must be stopped.';
...
begin
   ...
   if (Exec(ExpandConstant('{app}\Bin\Mailaholic.exe'), '/RegisterTypeLib', '',  SW_HIDE, ewWaitUntilTerminated, ResultCode) = False) then
...
begin
   try
      ...
      ProgressPage.SetText('Creating the Mailaholic service...', '');
      ProgressPage.SetProgress(3,6);

      // Register hMaillServer service
      if (Exec(ExpandConstant('{app}\Bin\Mailaholic.exe'), '/Register', '',  SW_HIDE, ewWaitUntilTerminated, ResultCode) = False) then
         MsgBox(SysErrorMessage(ResultCode), mbError, MB_OK);

      ProgressPage.SetText('Initializing Mailaholic database...', '');
      ...
      ProgressPage.SetText('Starting the Mailaholic service...', '');
...
begin
   ...
   sOldFile := ExpandConstant('{win}\Mailaholic.ini');
   sNewFile := ExpandConstant('{app}\Bin\Mailaholic.ini');

   // Copy the file from the Windows directory
   // to the Bin directory. Mailaholic uses the
   ...
   begin
      ...
      if (FileCopy(sOldFile, sNewFile, True) = True) then
      begin
        // We've managed to backup Mailaholic.ini in the
  ...
  if (GetCurrentDatabaseType() = 'mysql') and
    ...
    if (FileSize(szMySQLExecutable, iFileSize)) then begin
       ...
       if (iFileSize < 3500000) then begin
          ...
          szMessage := 'This version of Mailaholic does not include MySQL. Mailaholic can still' + #13 +
                       'use MySQL as backend though, assuming it is already installed on the system.' + #13 +
                       '' + #13 +
                       'However, the MySQL version Mailaholic is configured to use is'  + #13 +
                       'too old for this version of Mailaholic.' + #13 +
                       ''+ #13 +
                       'To solve this issue you must install the latest Mailaholic 4 version' + #13 +
                       'before upgrading to Mailaholic 5.' + #13
                       ''+ #13 +
                       'The latest Mailaholic 4 version will upgrade MySQL to an version' + #13 +
                       'which is compatible with Mailaholic 5.'
    ...
    begin
          szMessage := 'Mailaholic 5 and later does not include MySQL. Mailaholic 5 can still' + #13 +
                       'use MySQL as backend though, assuming it is already installed on the system.' + #13 +
                       '' + #13 +
                       'You have configured Mailaholic 4 to use the bundled MySQL installation. However'+ #13 +
                       'Mailaholic 4 with MySQL appears to have been uninstalled prior to running this' + #13 +
                       'Mailaholic 5 installation. Hence, the MySQL installation Mailaholic needs is' + #13 +
                       'no longer available.' + #13 +
                       '' + #13 +
                       'To solve this problem, reinstall the same Mailaholic 4 version as before and then' + #13 +
                       'upgrade to version 5 without first uninstalling version 4.' + #13 +
                       '' + #13 +
                       'As an alternative, you can cancel this installation, delete the entire Mailaholic ' + #13 +
...
begin
	...
	begin
		...
		if IsServiceRunning('Mailaholic') = true then
		begin
		 	 StopService('Mailaholic');
		
		   while (IsServiceStopped('Mailaholic') = false) do
		   begin
		      Sleep(250);
		   end;
		end;
...
begin
	...
	begin
	   // Move Mailaholic.ini before files are copied
	   MoveIni();
	end;
   ...
   // Create a registry key that tell
	  ...
	  RegWriteStringValue(HKLM32, 'Software\Mailaholic', 'InstallLocation', ExpandConstant('{app}'));
   	
	  // Write db location to Mailaholic.ini.
	  szIniFile := ExpandConstant('{app}\Bin\Mailaholic.ini');

  	// Create the Mailaholic database
