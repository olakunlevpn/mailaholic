...
void
InterfaceAccount::SetAuthentication(std::shared_ptr<MA::COMAuthentication> pAuthentication)
{
...
STDMETHODIMP InterfaceAccount::put_Password(BSTR newVal)
{
   try
   {
      ...
      // Mailaholic stores account passwords only as PBKDF2 (validation accepts
      // no other format), regardless of any legacy PreferredHashAlgorithm value.
      String sPassword = MA::Crypt::Instance()->EnCrypt(newVal, MA::Crypt::ETPBKDF2);

      object_->SetPassword(sPassword);
      object_->SetPasswordEncryption(MA::Crypt::ETPBKDF2);
...
STDMETHODIMP InterfaceAccount::get_Size(float *pVal)
{
   try
   {
      ...
      *pVal = MA::Math::Round((float) accountSizeBytes / fMB,3);
...
STDMETHODIMP InterfaceAccount::Save()
{
   try
   {
      ...
      {
         if (object_->GetAdminLevel() == MA::Account::ServerAdmin)
         {
      ...
      if (PersistentAccount::SaveObject(object_, sErrorMessage, true, MA::PersistenceModeNormal))
      {
...
STDMETHODIMP InterfaceAccount::ValidatePassword(BSTR Password, VARIANT_BOOL *pCorrect)
{
   try
   {
      ...
      *pCorrect = MA::PasswordValidator::ValidatePassword(object_, Password) ? VARIANT_TRUE : VARIANT_FALSE; 
      return S_OK;
   }
   catch (...)
   {
...
STDMETHODIMP InterfaceAccount::get_Messages(IInterfaceMessages **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Messages> pMsgs = object_->GetMessages();
...
STDMETHODIMP InterfaceAccount::get_FetchAccounts(IInterfaceFetchAccounts **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::FetchAccounts> pFetchAccounts = std::shared_ptr<MA::FetchAccounts>(new MA::FetchAccounts(object_->GetID()));
...
STDMETHODIMP InterfaceAccount::put_AdminLevel(eAdminLevel newVal)
{
   try
   {
      ...
      object_->SetAdminLevel((MA::Account::AdminLevel) newVal);
...
STDMETHODIMP InterfaceAccount::get_Rules(IInterfaceRules **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Rules> pRules = object_->GetRules();
...
STDMETHODIMP InterfaceAccount::get_IMAPFolders(IInterfaceIMAPFolders **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::IMAPFolders> pFolders = MA::IMAPFolderContainer::Instance()->GetFoldersForAccount(object_->GetID());
...
STDMETHODIMP InterfaceAccount::get_LastLogonTime(VARIANT *pVal)
{
   try
   {
      ...
      MA::DateTime dt = Time::GetDateFromSystemDate(object_->GetLastLogonTime());
