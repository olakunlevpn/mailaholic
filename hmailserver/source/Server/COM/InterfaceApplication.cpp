...
{
   authentication_ = std::shared_ptr<MA::COMAuthentication>(new MA::COMAuthentication);
...
STDMETHODIMP InterfaceApplication::Start()
{
   try
   {
      ...
      if (!MA::Application::Instance()->StartServers())
...
STDMETHODIMP InterfaceApplication::Stop()
{
   try
   {
      ...
      MA::Application::Instance()->StopServers();
...
STDMETHODIMP InterfaceApplication::Reinitialize()
{
   try
   {
      ...
      MA::String sErrorMessage = MA::Application::Instance()->Reinitialize();
...
STDMETHODIMP InterfaceApplication::get_ServerState(eServerState *pVal)
{
   try
   {
      ...
      MA::ServerStatus::ServerState iState = (MA::ServerStatus::ServerState) MA::ServerStatus::Instance()->GetState();
   
      switch (iState)
      {
      case MA::ServerStatus::StateStarting:
         *pVal = hStateStarting;
         break;
      case MA::ServerStatus::StateRunning:
         *pVal = hStateRunning;
         break;
      case MA::ServerStatus::StateStopping:
         *pVal = hStateStopping;
         break;
      case MA::ServerStatus::StateStopped:
         *pVal = hStateStopped;
         break;
      default:
         *pVal = hStateUnknown;
...
STDMETHODIMP InterfaceApplication::SubmitEMail()
{
   try
   {
      ...
      MA::ServiceManager pServiceManager;
...
STDMETHODIMP InterfaceApplication::Connect()
{
   try
   {
      MA::String sErrorMessage = MA::Application::Instance()->GetLastErrorMessage();
      if (!sErrorMessage.IsEmpty())
         return COMError::GenerateError(sErrorMessage);
...
STDMETHODIMP InterfaceApplication::Authenticate(BSTR sUsername, BSTR sPassword, IInterfaceAccount **pVal)
{
   try
   {
      ...
      std::shared_ptr<const MA::Account> pAccount = authentication_->Authenticate(sUsername, sPassword);
   
      if (pAccount)
      {
         std::shared_ptr<MA::Account> accountCopy = std::shared_ptr<MA::Account>(new MA::Account(*pAccount.get()));
...
STDMETHODIMP InterfaceApplication::get_Version(BSTR *pVal)
{
   try
   {
      *pVal = MA::Application::Instance()->GetVersionNumber().AllocSysString();
      return S_OK;
   }
   catch (...)
   {
...
STDMETHODIMP InterfaceApplication::get_VersionArchitecture(BSTR *pVal)
{
   try
   {
      *pVal = MA::Application::Instance()->GetVersionArchitecture().AllocSysString();
      return S_OK;
   }
   catch (...)
   {
...
STDMETHODIMP InterfaceApplication::get_InitializationFile(BSTR *pVal)
{
   try
   {
      ...
      *pVal = MA::IniFileSettings::Instance()->GetInitializationFile().AllocSysString();
...
STDMETHODIMP InterfaceApplication::get_Rules(IInterfaceRules **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Rules> pRules = std::shared_ptr<MA::Rules>(new MA::Rules(0));
...
{
   std::shared_ptr<MA::DatabaseConnectionManager> pConnectionManager = MA::Application::Instance()->GetDBManager();
