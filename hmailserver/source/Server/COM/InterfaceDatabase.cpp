...
InterfaceDatabase::LoadSettings()
{
   ...
   db_manager_ = MA::Application::Instance()->GetDBManager();
   ini_file_settings_ = MA::IniFileSettings::Instance();
   return true;
}
...
STDMETHODIMP InterfaceDatabase::get_IsConnected(VARIANT_BOOL *pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::DatabaseConnectionManager> pDBManager = MA::Application::Instance()->GetDBManager();
      if (pDBManager)
...
STDMETHODIMP InterfaceDatabase::get_DatabaseType(eDBtype *pVal)
{
   try
   {
      ...
      MA::DatabaseSettings::SQLDBType DBType = ini_file_settings_->GetDatabaseType();
      *pVal = (eDBtype) DBType;
...
STDMETHODIMP InterfaceDatabase::get_ServerName(BSTR *pVal)
{
   try
   {
      ...
      MA::String sServerName = ini_file_settings_->GetDatabaseServer();
...
STDMETHODIMP InterfaceDatabase::get_DatabaseName(BSTR *pVal)
{
   try
   {
      ...
      MA::String sServerName = ini_file_settings_->GetDatabaseName();
...
STDMETHODIMP InterfaceDatabase::CommitTransaction()
{
   try
   {
      ...
      std::shared_ptr<MA::DALConnection> pTempConn = conn_;
      conn_.reset();
...
STDMETHODIMP InterfaceDatabase::ExecuteSQLWithReturn(BSTR sSQLStatement, long *uniqueID)
{
   try
   {
      ...
      MA::String sStatement = sSQLStatement;
      ...
      if (conn_)
      {
         ...
         __int64 value = 0;
         if (conn_->TryExecute(MA::SQLCommand(sStatement), sErrorMessage, &value, 0) == MA::DALConnection::DALSuccess)
         {
            if (uniqueID)
               *uniqueID = (long) value;
      ...
      else
      {
         ...
         if (db_manager_->Execute(MA::SQLCommand(sStatement), &value, 0, sErrorMessage))
         {
            if (uniqueID)
               *uniqueID = (long) value;
...
STDMETHODIMP InterfaceDatabase::ExecuteSQLScript(BSTR sFilename)
{
   try
   {
      ...
      if (conn_)
      {
         MA::SQLScriptRunner scriptRunner;
...
STDMETHODIMP InterfaceDatabase::UtilGetFileNameByMessageID(hyper lMessageID, BSTR *Output)
{
   try
   {
      ...
      std::shared_ptr<MA::DALRecordset> pRS = db_manager_->OpenRecordset(MA::SQLCommand(sSQL));
      if (!pRS)
         return S_OK;
...
STDMETHODIMP InterfaceDatabase::CreateInternalDatabase()
{
   try
   {
      ...
      MA::String sDirectory = ini_file_settings_->GetDatabaseDirectory();
      MA::String sDatabaseName = "hMailServer";
      MA::String sPassword = MA::PasswordGenerator::Generate();
   
      MA::String sErrorMessage;
      if (!MA::SQLCEConnection::CreateDatabase(sDirectory, sDatabaseName, sPassword, sErrorMessage))
         return COMError::GenerateError(sErrorMessage);
   
      MA::String sEmpty;
   
      // Create a settings object which we use to connect to the server.
      std::shared_ptr<MA::DatabaseSettings> pSettings = std::shared_ptr<MA::DatabaseSettings>(
         new MA::DatabaseSettings(sEmpty, sEmpty, sDatabaseName, sEmpty, sPassword, sDirectory, sEmpty, MA::DatabaseSettings::TypeMSSQLCompactEdition, 0));
   
      // Connect to the new database
      std::shared_ptr<MA::DALConnection> pConn = MA::DALConnectionFactory::CreateConnection(pSettings);
   
      if (pConn->Connect(sErrorMessage) != MA::DALConnection::Connected)
      {
         return COMError::GenerateError(sErrorMessage);
      }
      ...
      ini_file_settings_->SetDatabaseType(MA::DatabaseSettings::TypeMSSQLCompactEdition);
   ...
   catch (_com_error &err)
   {
      ...
      MA::String sErrSource = lpcSource;

      LPCSTR lpcDesc = bstrDescription;
      MA::String sErrDesc = lpcDesc;
...
STDMETHODIMP InterfaceDatabase::CreateExternalDatabase(eDBtype ServerType, BSTR ServerName, long lPort, BSTR DatabaseName, BSTR Username, BSTR Password)
{
   try
   {
      ...
      const MA::String sEmpty;
      const MA::String sServerName = ServerName;
      const MA::String sDatabaseName = DatabaseName;
      const MA::String sUsername = Username;
      const MA::String sPassword = Password;
      const MA::String sProvider = ini_file_settings_->GetDatabaseProvider();
      ...
      std::shared_ptr<MA::DatabaseSettings> pSettings = std::shared_ptr<MA::DatabaseSettings>(
         new MA::DatabaseSettings(sProvider, sServerName, sEmpty, sUsername, sPassword, sEmpty, sEmpty, (MA::DatabaseSettings::SQLDBType) ServerType, lPort));
      ...
      // Create the database
      MA::String sCreateDB = MA::SQLStatement::GetCreateDatabase(pSettings, sDatabaseName);
   
      if (!pConn->Execute(MA::SQLCommand(sCreateDB), sErrorMessage, 0, 0))
         return COMError::GenerateError(sErrorMessage);
      ...
      pSettings = std::shared_ptr<MA::DatabaseSettings>(
         new MA::DatabaseSettings(sProvider, sServerName, sDatabaseName, sUsername, sPassword, sEmpty, sEmpty, (MA::DatabaseSettings::SQLDBType) ServerType, lPort));
   
      // Reconnect to the new database.
      pConn = MA::DALConnectionFactory::CreateConnection(pSettings);
      if (pConn->Connect(sErrorMessage) != MA::DALConnection::Connected)
         return COMError::GenerateError(sErrorMessage);
      ...
      ini_file_settings_->SetDatabaseType((MA::DatabaseSettings::SQLDBType) ServerType);
...
STDMETHODIMP InterfaceDatabase::SetDefaultDatabase(eDBtype ServerType, BSTR ServerName, long lPort, BSTR DatabaseName, BSTR Username, BSTR Password)
{
   try
   {
      ...
      MA::String sEmpty;
      MA::String sServerName = ServerName;
      MA::String sDatabaseName = DatabaseName;
      MA::String sUsername = Username;
      MA::String sPassword = Password;
   
      if (sDatabaseName.Find(_T(" ")) >= 0)
         return COMError::GenerateError("The database name may not contain spaces.");
   
      // Create a settings object for the connection ...
      MA::String sProvider = ini_file_settings_->GetDatabaseProvider();
...
STDMETHODIMP InterfaceDatabase::EnsurePrerequisites(long DBVersion)
{
   try
   {
      ...
      if (conn_)
      {
         MA::PrerequisiteList prerequisites;
