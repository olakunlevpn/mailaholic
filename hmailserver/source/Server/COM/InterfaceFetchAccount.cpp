...
{
   object_ = std::shared_ptr<MA::FetchAccount>(new MA::FetchAccount());
}
...
STDMETHODIMP InterfaceFetchAccount::put_ServerType(LONG newVal)
{
   try
   {
      ...
      object_->SetServerType((MA::FetchAccount::ServerType) newVal);
...
STDMETHODIMP InterfaceFetchAccount::Save(void)
{
   try
   {
      ...
      MA::PersistentFetchAccount oPersAccount;
...
STDMETHODIMP InterfaceFetchAccount::DownloadNow(void)
{
   try
   {
      ...
      MA::PersistentFetchAccount::SetRetryNow(object_->GetID());
   
      // Notify the fetch manager to look for accounts to fetch from now.
      MA::Application::Instance()->GetExternalFetchManager()->SetCheckNow();
...
STDMETHODIMP InterfaceFetchAccount::get_UseSSL(VARIANT_BOOL *pVal)
{
   try
   {
      ...
      *pVal = object_->GetConnectionSecurity() == MA::CSSSL ? VARIANT_TRUE : VARIANT_FALSE;
...
STDMETHODIMP InterfaceFetchAccount::put_UseSSL(VARIANT_BOOL newVal)
{
   try
   {
      ...
      if (newVal == VARIANT_TRUE)
         object_->SetConnectionSecurity(MA::CSSSL);
      else
         object_->SetConnectionSecurity(MA::CSNone);
...
STDMETHODIMP InterfaceFetchAccount::put_ConnectionSecurity(eConnectionSecurity newVal)
{
   try
   {
      ...
      object_->SetConnectionSecurity((MA::ConnectionSecurity) newVal);
      return S_OK;
   }
   catch (...)
   {
...
STDMETHODIMP InterfaceFetchAccount::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentFetchAccount::DeleteObject(object_) ? S_OK : S_FALSE;
...
STDMETHODIMP InterfaceFetchAccount::get_IsLocked(VARIANT_BOOL* pVal)
{
	try
	{
		...
		bool locked = MA::PersistentFetchAccount::IsLocked(object_->GetID());
