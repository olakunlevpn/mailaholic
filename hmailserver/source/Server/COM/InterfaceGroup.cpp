...
InterfaceGroup::Save()
{
   try
   {
      ...
      if (MA::PersistentGroup::SaveObject(object_, sErrorMessage, MA::PersistenceModeNormal))
      {
...
STDMETHODIMP InterfaceGroup::get_Members(IInterfaceGroupMembers **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::GroupMembers> pGM = object_->GetMembers();
...
STDMETHODIMP InterfaceGroup::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentGroup::DeleteObject(object_) ? S_OK : S_FALSE;
