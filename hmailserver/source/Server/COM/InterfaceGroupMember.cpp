...
InterfaceGroupMember::Save()
{
   try
   {
      ...
      if (MA::PersistentGroupMember::SaveObject(object_))
      {
...
InterfaceGroupMember::get_Account(IInterfaceAccount **pVal)
{
   try
   {
      ...
      if (!MA::PersistentAccount::ReadObject(pAccount, (__int64) object_->GetAccountID()))
...
STDMETHODIMP InterfaceGroupMember::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentGroupMember::DeleteObject(object_) ? S_OK : S_FALSE;
