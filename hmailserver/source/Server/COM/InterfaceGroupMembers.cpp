...
void 
InterfaceGroupMembers::Attach(std::shared_ptr<MA::GroupMembers> pBA) 
{ 
   group_members_ = pBA; 
}
...
InterfaceGroupMembers::get_Item(long Index, IInterfaceGroupMember **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::GroupMember> pBA = group_members_->GetItem(Index);
...
InterfaceGroupMembers::get_ItemByDBID(long lDBID, IInterfaceGroupMember **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::GroupMember> pBA = group_members_->GetItemByDBID(lDBID);
...
InterfaceGroupMembers::Add(IInterfaceGroupMember **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::GroupMember> pBA = std::shared_ptr<MA::GroupMember>(new MA::GroupMember);
