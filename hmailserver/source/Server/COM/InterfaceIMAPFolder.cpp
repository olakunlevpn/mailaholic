...
STDMETHODIMP InterfaceIMAPFolder::Save()
{
   try
   {
      ...
      if (MA::PersistentIMAPFolder::SaveObject(object_))
      {
...
void 
InterfaceIMAPFolder::Attach(std::shared_ptr<MA::IMAPFolder> pFolder)
{
   object_ = pFolder;
}
...
InterfaceIMAPFolder::get_Name(BSTR *pVal)
{
   try
   {
      ...
      MA::String sUnicode = MA::ModifiedUTF7::Decode(object_->GetFolderName());
...
STDMETHODIMP InterfaceIMAPFolder::put_Name(BSTR newVal)
{
   try
   {
      ...
      object_->SetFolderName(MA::ModifiedUTF7::Encode(newVal));
      return S_OK;
   }
   catch (...)
   {
...
InterfaceIMAPFolder::get_Messages(IInterfaceMessages **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Messages> pMessages = object_->GetMessages();
...
InterfaceIMAPFolder::get_SubFolders(IInterfaceIMAPFolders **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::IMAPFolders> pFolders = object_->GetSubFolders();
...
STDMETHODIMP InterfaceIMAPFolder::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentIMAPFolder::DeleteObject(object_) ? S_OK : S_FALSE;
...
InterfaceIMAPFolder::get_CreationTime(BSTR *pVal)
{
   try
   {
      ...
      *pVal = MA::Time::GetTimeStampFromDateTime(object_->GetCreationTime()).AllocSysString();
      return S_OK;
   }
   catch (...)
   {
