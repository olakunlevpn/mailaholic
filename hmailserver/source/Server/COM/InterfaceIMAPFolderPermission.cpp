...
STDMETHODIMP InterfaceIMAPFolderPermission::put_PermissionType(eACLPermissionType newVal)
{
   try
   {
      ...
      object_->SetPermissionType((MA::ACLPermission::ePermissionType) newVal);
      return S_OK;
   }
   catch (...)
   {
...
STDMETHODIMP InterfaceIMAPFolderPermission::Save()
{
   try
   {
      ...
      if (MA::PersistentACLPermission::SaveObject(object_))
      {
...
InterfaceIMAPFolderPermission::get_Permission(eACLPermission iPermission, VARIANT_BOOL *pVal)
{
   try
   {
      ...
      *pVal = object_->GetAllow((MA::ACLPermission::ePermission) iPermission) ? VARIANT_TRUE : VARIANT_FALSE;
      return S_OK;
   }
   catch (...)
   {
...
InterfaceIMAPFolderPermission::put_Permission(eACLPermission iPermission, VARIANT_BOOL newVal)
{
   try
   {
      ...
      object_->SetAllow((MA::ACLPermission::ePermission) iPermission, newVal == VARIANT_TRUE);
      return S_OK;
   }
   catch (...)
   {
...
STDMETHODIMP InterfaceIMAPFolderPermission::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentACLPermission::DeleteObject(object_) ? S_OK : S_FALSE;
...
InterfaceIMAPFolderPermission::get_Account(IInterfaceAccount **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Account> pAccount = std::shared_ptr<MA::Account>(new MA::Account);
   
      if (!MA::PersistentAccount::ReadObject(pAccount, (__int64) object_->GetPermissionAccountID()))
...
InterfaceIMAPFolderPermission::get_Group(IInterfaceGroup **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Group> pGroup = MA::Configuration::Instance()->GetIMAPConfiguration()->GetGroups()->GetItemByDBID(object_->GetPermissionGroupID());
