...
void
InterfaceIMAPFolders::Attach(std::shared_ptr<MA::IMAPFolders> pFolders)
{
   object_ = pFolders;
}
...
InterfaceIMAPFolders::get_Item(long Index, IInterfaceIMAPFolder **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::IMAPFolder> pIMAPFolder = object_->GetItem(Index);
...
InterfaceIMAPFolders::get_ItemByDBID(long DBID, IInterfaceIMAPFolder **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::IMAPFolder> pIMAPFolder = object_->GetItemByDBID(DBID);
...
InterfaceIMAPFolders::get_ItemByName(BSTR sName, IInterfaceIMAPFolder **pVal)
{
   try
   {
      ...
      MA::String sUnicode (sName);
      MA::AnsiString sFolderName = MA::ModifiedUTF7::Encode(sUnicode);
   
      std::shared_ptr<MA::IMAPFolder> pIMAPFolder = object_->GetFolderByName(sFolderName);
...
InterfaceIMAPFolders::Add(BSTR sName, IInterfaceIMAPFolder **pVal)
{
   try
   {
      ...
      pIMAPFolder = std::shared_ptr<MA::IMAPFolder>(new MA::IMAPFolder(object_->GetAccountID(), object_->GetParentID()));
      ...
      if (!MA::PersistentIMAPFolder::SaveObject(pIMAPFolder))
      {
