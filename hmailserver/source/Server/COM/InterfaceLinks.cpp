...
STDMETHODIMP InterfaceLinks::get_Domain(long DBID, IInterfaceDomain **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Domain> pDomain = std::shared_ptr<MA::Domain>(new MA::Domain);
      if (!MA::PersistentDomain::ReadObject(pDomain, DBID))
...
STDMETHODIMP InterfaceLinks::get_Account(long DBID, IInterfaceAccount **pVal)
{
   try
   {
      ...
      if (!MA::PersistentAccount::ReadObject(pAccount, DBID))
         return DISP_E_BADINDEX;
...
STDMETHODIMP InterfaceLinks::get_Alias(long DBID, IInterfaceAlias **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Alias> pAlias = std::shared_ptr<MA::Alias>(new MA::Alias);
      if (!MA::PersistentAlias::ReadObject(pAlias, DBID))
         return DISP_E_BADINDEX;
...
STDMETHODIMP InterfaceLinks::get_DistributionList(long DBID, IInterfaceDistributionList **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::DistributionList> pList = std::shared_ptr<MA::DistributionList>(new MA::DistributionList);
      if (!MA::PersistentDistributionList::ReadObject(pList, DBID))
         return DISP_E_BADINDEX;
