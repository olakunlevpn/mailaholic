...
{
   object_ = std::shared_ptr<MA::Message>(new MA::Message());
}
...
STDMETHODIMP InterfaceMessage::get_InternalDate(VARIANT *pVal)
{
   try
   {
      ...
      MA::DateTime dt = MA::Time::GetDateFromSystemDate(object_->GetCreateTime());
...
STDMETHODIMP InterfaceMessage::get_Filename(BSTR *pVal)
{
   try
   {
      ...
      std::shared_ptr<const MA::Account> account;
      
      if (object_->GetAccountID() > 0)
      {
         account = MA::CacheContainer::Instance()->GetAccount(object_->GetAccountID());
      }
   
      *pVal = MA::PersistentMessage::GetFileName(account, object_).AllocSysString();
...
STDMETHODIMP InterfaceMessage::get_Attachments(IInterfaceAttachments **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Attachments> pAttachments = GetMessageData_()->GetAttachments();
...
STDMETHODIMP InterfaceMessage::get_Headers(IInterfaceMessageHeaders **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::MimeBody> pMimeBody = GetMessageData_()->GetMimeMessage();
...
InterfaceMessage::SaveNewMessageToIMAPFolder_()
{
   try
   {
      ...
      std::shared_ptr<const MA::Account> pAccount = MA::CacheContainer::Instance()->GetAccount(object_->GetAccountID());
   
      // Save the message to the database
      if (!MA::PersistentMessage::SaveObject(object_))
         return S_FALSE;
   
      // Add to parent collection
      MA::MessagesContainer::Instance()->SetFolderNeedsRefresh(object_->GetFolderID());
   
      // Notify...
      std::shared_ptr<MA::ChangeNotification> pNotification = 
         std::shared_ptr<MA::ChangeNotification>(new MA::ChangeNotification(object_->GetAccountID(), object_->GetFolderID(), MA::ChangeNotification::NotificationMessageAdded));
   
      MA::Application::Instance()->GetNotificationServer()->SendNotification(pNotification);
...
STDMETHODIMP InterfaceMessage::Save()
{
   try
   {
      ...
      MA::String sDate = GetMessageData_()->GetSentTime();
      if (sDate.IsEmpty())
      {
         // Date was not specified. Specify it now.
         sDate = MA::Time::GetCurrentMimeDate();
      ...
      const MA::String fileName = MA::PersistentMessage::GetFileName(account, object_);
      ...
      MA::Message::State state = object_->GetState();
   
      switch (state)
      {
      case MA::Message::Created:
         {
            ...
            if (object_->GetFolderID() == 0 && object_->GetAccountID() == 0)
            {
               ...
               object_->SetState(MA::Message::Delivering);
...
STDMETHODIMP InterfaceMessage::Copy(long lDestinationFolderID)
{
   try
   {
      ...
      bool bSuccess = MA::MessageUtilities::CopyToIMAPFolder(object_, lDestinationFolderID);
      if (!bSuccess)
      {
...
STDMETHODIMP InterfaceMessage::AddRecipient(BSTR bstrName, BSTR bstrAddress)
{
   try
   {
      ...
      MA::String sAddress(bstrAddress);
      MA::String sName (bstrName);
   
      bool recipientOK = false;
      MA::RecipientParser recipientParser;
      ...
      MA::String sThisAddress = "\"" + sName + "\"" + " <" + sAddress + ">";
      MA::String sTo = GetMessageData_()->GetTo();
...
STDMETHODIMP InterfaceMessage::get_Recipients(IInterfaceRecipients**pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Message> pMessage = GetMessageData_()->GetMessage();
...
InterfaceMessage::GetMessageData_()
{
   if (!msg_data_)
   {
      ...
      msg_data_ = std::shared_ptr<MA::MessageData>(new MA::MessageData());
