...
void
InterfaceMessages::Attach(std::shared_ptr<MA::Messages> pMessages)
{
   messages_ = pMessages;
}
...
STDMETHODIMP InterfaceMessages::get_Item(long Index, IInterfaceMessage **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Message> pMsg = messages_->GetItem(Index);
...
STDMETHODIMP InterfaceMessages::get_ItemByDBID(hyper DBID, IInterfaceMessage **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Message> pMsg = messages_->GetItemByDBID(DBID);
...
STDMETHODIMP InterfaceMessages::Add(IInterfaceMessage **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Message> pNewMsg = std::shared_ptr<MA::Message>(new MA::Message);
      pNewMsg->SetAccountID(iAccountID);
      pNewMsg->SetFolderID(iFolderID);
      pNewMsg->SetState(MA::Message::Created);
...
STDMETHODIMP InterfaceMessages::DeleteByDBID(hyper lDBID)
{
   try
   {
      ...
      std::shared_ptr<MA::Message> pMsg = messages_->GetItemByDBID(lDBID);
      ...
      std::function<bool(int, std::shared_ptr<MA::Message>)> filter = [lDBID](int index, std::shared_ptr<MA::Message> message)
      {
      ...
      if (iFolderID == -1)
      {
         ...
         std::shared_ptr<MA::ChangeNotification> notification =
            std::shared_ptr<MA::ChangeNotification>(new MA::ChangeNotification(messages_->GetAccountID(), messages_->GetFolderID(), MA::ChangeNotification::NotificationMessageDeleted, affectedMessages));

         MA::Application::Instance()->GetNotificationServer()->SendNotification(notification);
      }
