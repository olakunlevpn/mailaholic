...
void
InterfaceRecipients::Attach(std::shared_ptr<MA::Message> pMessage)
{
   message_ = pMessage;
...
InterfaceRecipients::get_Count(long *pVal)
{
   try
   {
      ...
      std::vector<std::shared_ptr<MA::MessageRecipient> > vecRecipients = message_->GetRecipients()->GetVector();
...
InterfaceRecipients::get_Item(long Index, IInterfaceRecipient **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::MessageRecipient> pRecipient = vecRecipients[Index];
