...
STDMETHODIMP InterfaceRule::get_Criterias(IInterfaceRuleCriterias **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::RuleCriterias> pRuleCriterias = object_->GetCriterias();
...
STDMETHODIMP InterfaceRule::get_Actions(IInterfaceRuleActions **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::RuleActions> pRuleActions = object_->GetActions();
...
STDMETHODIMP InterfaceRule::Save()
{
   try
   {
      ...
      {
         std::vector<std::shared_ptr<MA::Rule> > vecExistingRules = parent_collection_->GetVector();
         ...
         {
            std::shared_ptr<MA::Rule> pLastRule = vecExistingRules[vecExistingRules.size() -1];
            object_->SetSortOrder(pLastRule->GetSortOrder() +1);
         }
      }
   
      if (MA::PersistentRule::SaveObject(object_))
      {
...
STDMETHODIMP InterfaceRule::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentRule::DeleteObject(object_) ? S_OK : S_FALSE;
