...
STDMETHODIMP InterfaceRuleAction::Save()
{
   try
   {
      ...
      {
         std::vector<std::shared_ptr<MA::RuleAction> > vecExistingRuleActions = parent_collection_->GetVector();
         ...
         {
            std::shared_ptr<MA::RuleAction> pLastRuleAction = vecExistingRuleActions[vecExistingRuleActions.size() -1];
      ...
      {
         if (!MA::PersistentRuleAction::SaveObject(object_))
         {
            // Saving failed.
...
STDMETHODIMP InterfaceRuleAction::put_Type(eRuleActionType newVal)
{
   try
   {
      ...
      object_->SetType((MA::RuleAction::Type) newVal);
...
STDMETHODIMP InterfaceRuleAction::get_IMAPFolder(BSTR *pVal)
{
   try
   {
      ...
      MA::String sValue = object_->GetIMAPFolder();
      MA::String sResult = MA::ModifiedUTF7::Decode(T2A(sValue.GetBuffer()));
...
STDMETHODIMP InterfaceRuleAction::put_IMAPFolder(BSTR newVal)
{
   try
   {
      ...
      MA::String sUnicode (newVal);
   
      // Convert the unciode string to a modified UTF7.
      // Since hMailServer core don't know unicode, we
      // have to do it out here in the interface.
      MA::AnsiString sRetVal = MA::ModifiedUTF7::Encode(sUnicode);
...
STDMETHODIMP InterfaceRuleAction::Delete()
{
   try
   {
      ...
      if (!parent_collection_)
         return MA::PersistentRuleAction::DeleteObject(object_) ? S_OK : S_FALSE;
