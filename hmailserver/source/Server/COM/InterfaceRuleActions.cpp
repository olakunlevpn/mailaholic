...
void 
InterfaceRuleActions::Attach(std::shared_ptr<MA::RuleActions> pActions)
{
...
STDMETHODIMP InterfaceRuleActions::get_ItemByDBID(long lDBID, IInterfaceRuleAction** pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::RuleAction> pRule = rule_actions_->GetItemByDBID(lDBID);
      if (!pRule)
...
STDMETHODIMP InterfaceRuleActions::get_Item(long lIndex, IInterfaceRuleAction** pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::RuleAction> pRule = rule_actions_->GetItem(lIndex);
      if (!pRule)
         return DISP_E_BADINDEX;
...
STDMETHODIMP InterfaceRuleActions::Add(IInterfaceRuleAction** pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::RuleAction> pRuleAction = std::shared_ptr<MA::RuleAction>(new MA::RuleAction);
