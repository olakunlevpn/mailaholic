...
void 
InterfaceRules::Attach(std::shared_ptr<MA::Rules> pRules)
{
   rules_ = pRules;
}
...
STDMETHODIMP InterfaceRules::get_ItemByDBID(long lDBID, IInterfaceRule** pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Rule> pRule = rules_->GetItemByDBID(lDBID);
      if (!pRule)
...
STDMETHODIMP InterfaceRules::get_Item(long lIndex, IInterfaceRule** pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Rule> pRule = rules_->GetItem(lIndex);
      if (!pRule)
         return DISP_E_BADINDEX;
...
STDMETHODIMP InterfaceRules::Add(IInterfaceRule** pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::Rule> pDA = std::shared_ptr<MA::Rule>(new MA::Rule);
