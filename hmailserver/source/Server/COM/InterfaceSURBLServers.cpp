...
InterfaceSURBLServers::LoadSettings()
{
   ...
   surbl_servers_ = MA::Configuration::Instance()->GetAntiSpamConfiguration().GetSURBLServers();
...
InterfaceSURBLServers::get_Item(long Index, IInterfaceSURBLServer **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::SURBLServer> pDNSBlackList = surbl_servers_->GetItem(Index);
...
InterfaceSURBLServers::get_ItemByDBID(long lDBID, IInterfaceSURBLServer **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::SURBLServer> pDNSBlackList = surbl_servers_->GetItemByDBID(lDBID);
...
InterfaceSURBLServers::Add(IInterfaceSURBLServer **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::SURBLServer> pDNSBL = std::shared_ptr<MA::SURBLServer>(new MA::SURBLServer);
...
InterfaceSURBLServers::get_ItemByDNSHost(BSTR ItemName, IInterfaceSURBLServer **pVal)
{
   try
   {
      ...
      std::shared_ptr<MA::SURBLServer> pDNSBL = surbl_servers_->GetItemByName(ItemName);
      if (!pDNSBL)
         return S_FALSE;
