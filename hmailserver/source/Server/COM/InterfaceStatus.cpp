...
InterfaceStatus::LoadSettings()
{
   ...
   status_ = MA::ServerStatus::Instance();
   application_ = MA::Application::Instance();
...
InterfaceStatus::get_UndeliveredMessages(BSTR *pVal)
{
   try
   {
      ...
      MA::String sRetVal = status_->GetUnsortedMessageStatus();
...
InterfaceStatus::get_StartTime(BSTR *pVal)
{
   try
   {
      ...
      MA::String sRetVal = application_->GetStartTime();
      *pVal = sRetVal.AllocSysString();
      return S_OK;
   }
   catch (...)
