...
namespace MA
{
   ...
   DKIM::Initialize()
   {
      // (OpenSSL 3.x loads error strings automatically; no explicit call needed.)
   ...
   DKIM::SignHash_(AnsiString &privateKey, AnsiString &canonicalizedHeader, HashCreator::HashType hashType)
   {
      ...
      // RFC 8301: always sign with SHA-256 (the hashType argument is ignored).
      EVP_SignInit( headerSigningContext, EVP_sha256());
   ...
   */
   ...
   DKIM::VerifySignature_(const String &fileName, const AnsiString &messageHeader, std::pair<AnsiString, AnsiString> signatureField)
   {
      ...
      */
      ...
      // Only rsa-sha256 reaches here (ValidateHeaderContents_ rejects the rest).
      HashCreator shaer(HashCreator::SHA256);
   ...
   DKIM::VerifyHeaderHash_(AnsiString canonicalizedHeader, const AnsiString &tagA, AnsiString &tagB, const AnsiString &publicKeyString)
   {
      ...
      // RFC 8301: rsa-sha256 only. rsa-sha1 is rejected before reaching here.
   ...
   DKIM::ValidateHeaderContents_(const DKIMParameters &signatureParams)
   {
      ...
      */
      ...
      // RFC 8301: verifiers MUST NOT consider rsa-sha1 signatures valid. Only
      // rsa-sha256 is accepted; any other algorithm (including rsa-sha1) fails.
      if (tagA != "rsa-sha256")
      {
         LOG_DEBUG("DKIM: Unsupported signing algorithm (RFC 8301 requires rsa-sha256). Aborting DKIM test.");
         return false;
      }
