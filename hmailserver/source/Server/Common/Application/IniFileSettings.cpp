...
namespace MA
{
   ...
   IniFileSettings::IniFileSettings() :
      ...
      mxtries_factor_(0),
      sqldbtype_(MA::DatabaseSettings::TypeUnknown),
      sep_svc_logs_(false),
   ...
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Load all settings from Mailaholic.ini
   //---------------------------------------------------------------------------()
   {
      ...
      if (sDatabaseType.CompareNoCase(_T("MSSQL")) == 0)
         sqldbtype_ = MA::DatabaseSettings::TypeMSSQLServer;
      else if (sDatabaseType.CompareNoCase(_T("MYSQL")) == 0)
         sqldbtype_ = MA::DatabaseSettings::TypeMYSQLServer;
      else if (sDatabaseType.CompareNoCase(_T("PostgreSQL")) == 0)
         sqldbtype_ = MA::DatabaseSettings::TypePGServer;
      else if (sDatabaseType.CompareNoCase(_T("MSSQLCE")) == 0)
         sqldbtype_ = MA::DatabaseSettings::TypeMSSQLCompactEdition;
      ...
      if (sqldbtype_ == MA::DatabaseSettings::TypeMSSQLCompactEdition)
      {
   ...
   {
      if (sqldbtype_ == MA::DatabaseSettings::TypeUnknown)
         return false;
   ...
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Updates the main Mailaholic administration password found in Mailaholic.ini
   //---------------------------------------------------------------------------()
   {
      administrator_password_ = MA::Crypt::Instance()->EnCrypt(sNewPassword, MA::Crypt::ETPBKDF2);

      WriteIniSetting_("Security", "AdministratorPassword", administrator_password_);
   }

   String
   IniFileSettings::GetSecretKey()
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Returns the AES-256 key (64 hex chars) for reversible relay/fetch secrets.
   // Reads it directly from the INI so it works before LoadSettings() has run;
   // generates and persists a random key on first use. Fails closed if the
   // random source is unavailable (never returns a weak/constant key).
   //---------------------------------------------------------------------------()
   {
      static std::mutex secretKeyMutex;
      std::lock_guard<std::mutex> guard(secretKeyMutex);

      // Return the key cached for this process run, if any.
      if (!secret_key_.IsEmpty())
         return secret_key_;

      // Otherwise read it from the INI.
      String sKey = ReadIniSettingString_("Security", "SecretKey", "");
      if (!sKey.IsEmpty())
      {
         secret_key_ = sKey;
         return secret_key_;
      ...
      // First use: generate a 32-byte random key and hex-encode it.
      unsigned char raw[32];
      if (RAND_bytes(raw, sizeof(raw)) != 1)
         throw std::runtime_error("RAND_bytes failed generating SecretKey");

      String sHex;
      for (int i = 0; i < (int)sizeof(raw); i++)
      {
         String sByte;
         sByte.Format(_T("%02x"), raw[i]);
         sHex += sByte;
      }

      WriteIniSetting_("Security", "SecretKey", sHex);

      // Verify it persisted. A silent write failure (read-only/locked INI)
      // would otherwise make every restart generate a fresh key and
      // permanently orphan any previously-encrypted relay/fetch secrets.
      String sVerify = ReadIniSettingString_("Security", "SecretKey", "");
      if (sVerify != sHex)
         throw std::runtime_error("Failed to persist SecretKey to the configuration file");
   ...
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Updates a directory in Mailaholic.ini.
   //---------------------------------------------------------------------------()
   {
   ...
   void 
   IniFileSettings::SetDatabaseType(MA::DatabaseSettings::SQLDBType type)
   //---------------------------------------------------------------------------()
   // DESCRIPTION:
   // Updates a directory in Mailaholic.ini.
   //---------------------------------------------------------------------------()
