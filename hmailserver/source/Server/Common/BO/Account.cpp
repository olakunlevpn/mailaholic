...
namespace MA
{
   ...
   Account::GetFolders()
   {
      ...
      {
         folders_ = std::shared_ptr<IMAPFolders>(new MA::IMAPFolders(id_, -1));
         folders_->Refresh();
   ...
   Account::XMLStore(XNode *pParentNode, int iBackupOptions)
   {
      ...
      std::shared_ptr<MA::FetchAccounts> pFetchAccounts = std::shared_ptr<MA::FetchAccounts>(new MA::FetchAccounts(id_));
