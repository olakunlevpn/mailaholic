...
namespace MA
{
   ...
   FetchAccount::XMLStore(XNode *pRuleNode, int iOptions)
   {
      ...
      pNode->AppendAttr(_T("Password"), Crypt::Instance()->EnCrypt(password_, Crypt::ETAESGCM));
   ...
   FetchAccount::XMLLoad(XNode *pNode, int iOptions)
   {
      ...
      password_ = Crypt::Instance()->DeCrypt(pNode->GetAttrValue(_T("Password")), Crypt::ETAESGCM);
