...
namespace MA
{
   ...
   Route::XMLStore(XNode *pRoutesNode, int iOptions)
   {
      ...
      pNode->AppendAttr(_T("Password"), Crypt::Instance()->EnCrypt(relayer_auth_password_, Crypt::ETAESGCM));
   ...
   Route::XMLLoad(XNode *pNode, int iOptions)
   {
      ...
      relayer_auth_password_ = Crypt::Instance()->DeCrypt(pNode->GetAttrValue(_T("Password")), Crypt::ETAESGCM);
