...
namespace MA   
{
   ...
   PersistentAccount::ReadObject(std::shared_ptr<Account> pAccount, std::shared_ptr<DALRecordset> pRS)
   {
      ...
      {
         // Plain-text password supplied; store it as PBKDF2 (the only format
         // Mailaholic validates), regardless of any legacy preferred-algorithm setting.
         pAccount->SetPassword(Crypt::Instance()->EnCrypt(pAccount->GetPassword(), Crypt::ETPBKDF2));
         pAccount->SetPasswordEncryption(Crypt::ETPBKDF2);
      }
