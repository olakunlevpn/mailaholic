...
namespace MA
{
   ...
   PersistentFetchAccount::ReadObject(std::shared_ptr<FetchAccount> oFA, std::shared_ptr<DALRecordset> pRS)
   {
      ...
      oFA->SetServerType((MA::FetchAccount::ServerType)pRS->GetLongValue("faservertype"));
      oFA->SetUsername(pRS->GetStringValue("fausername"));
      oFA->SetPassword(Crypt::Instance()->DeCrypt(pRS->GetStringValue("fapassword"), Crypt::ETAESGCM));
   ...
   PersistentFetchAccount::SaveObject(std::shared_ptr<FetchAccount> pFA)
   {
      ...
      oStatement.AddColumn("fapassword", Crypt::Instance()->EnCrypt(pFA->GetPassword(), Crypt::ETAESGCM));
