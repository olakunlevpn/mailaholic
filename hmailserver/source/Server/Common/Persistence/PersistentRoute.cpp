...
namespace MA
{
   ...
   PersistentRoute::SaveObject(std::shared_ptr<Route> pRoute, String &sErrorMessage, PersistenceMode mode)
   {
      ...
      oStatement.AddColumn("routeauthenticationpassword", Crypt::Instance()->EnCrypt(pRoute->GetRelayerAuthPassword(), Crypt::ETAESGCM));
   ...
   PersistentRoute::ReadObject(std::shared_ptr<Route> pRoute, std::shared_ptr<DALRecordset> pRS)
   {
      ...
      pRoute->SetRelayerAuthPassword(Crypt::Instance()->DeCrypt(pRS->GetStringValue("routeauthenticationpassword"), Crypt::ETAESGCM));
