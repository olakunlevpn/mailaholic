...
namespace MA
{
   ...
   SQLStatement::GetCommand() const
   {
      ...
      if (type_ == SQLStatement::STSelect)
      {
         if (top_rows_ > -1)
         {
            MA::DatabaseSettings::SQLDBType DBType = IniFileSettings::Instance()->GetDatabaseType();
            String value;
            switch (DBType)
            {
            case DatabaseSettings::TypePGServer:
   ...
   {
      MA::DatabaseSettings::SQLDBType DBType = pSettings->GetType();
      String sSQL;
      switch (DBType)
      {
      case DatabaseSettings::TypeMSSQLServer:
   ...
   SQLStatement::Escape(const String &input)
   {
      ...
      MA::DatabaseSettings::SQLDBType iType = IniFileSettings::Instance()->GetDatabaseType();
