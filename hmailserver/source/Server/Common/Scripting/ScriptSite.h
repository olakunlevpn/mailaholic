...
public:
   ...
   STDMETHOD(HandleScriptError)(EXCEPINFO* pei,ULONG ulLine,LONG lPos,BSTR src)
   {
      ...
      MA::Logger::Instance()->LogError(sMsg);
   ...
   MA::String GetLastError()
   {
      return last_error_message_;
   }

   protected:
      std::shared_ptr<MA::ScriptObjectContainer> object_container_;

      MA::String last_error_message_;
};
...
public:
   ...
   bool 
   ProcedureExists(MA::String sName)
   {
   ...
   STDMETHOD(SetObjectContainer)(std::shared_ptr<MA::ScriptObjectContainer> pObject)
   {
      object_container_ = pObject;

      // Add the objects to namespace
      std::vector<MA::String> vecNames = object_container_->GetObjectNames();
      
      for(MA::String name : vecNames)
      {
         AddObject(name, TRUE);
      }
