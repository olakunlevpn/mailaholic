...
namespace MA
{
   ...
   {
      SetContextOptions_(context, false);
   ...
   void
   SslContextInitializer::SetContextOptions_(boost::asio::ssl::context& context, bool serverContext)
   {
      ...
      // SSLv2/SSLv3 are always disabled.
      int options = SSL_OP_ALL | SSL_OP_SINGLE_DH_USE | SSL_OP_SINGLE_ECDH_USE |
                    SSL_OP_NO_SSLv2 | SSL_OP_NO_SSLv3;

      if (serverContext)
      {
         // Inbound connections: enforce a TLS 1.2 minimum regardless of admin
         // settings. Outbound (client) connections stay permissive so
         // opportunistic TLS to legacy peers still succeeds rather than falling
         // back to cleartext delivery.
         options = options | SSL_OP_NO_TLSv1 | SSL_OP_NO_TLSv1_1;
      }
      ...
      if (serverContext)
      {
         // Inbound only: OpenSSL security level 2 rejects < 112-bit security
         // (SHA1 signatures, RSA/DH keys < 2048 bits, weak cipher suites).
