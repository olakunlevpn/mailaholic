...
namespace MA
{
   ...
   ExceptionLogger::Log(int exception_code, EXCEPTION_POINTERS* pExp)
   {
      ...
      String command_line = "Mailaholic.minidump.exe";
      ...
      {
         String message = Formatter::Format("An error has been detected. hMailServer attempted to generate minidump, but Mailaholic.minidump.exe returned {0}.", exit_code);
