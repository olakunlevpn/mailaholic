...
namespace MA
{
   ...
   {  
      ...
      if (sender_domain->GetEnableSignature())
      {
         ...
         // keep it.
         if (sender_domain->GetSignatureMethod() == MA::Domain::SMSetIfNotSpecifiedInAccount)
         {
         ...
         else if (sender_domain->GetSignatureMethod() == MA::Domain::SMAppendToAccountSignature)
         {
            signature_plain_text += "\r\n\r\n" + sDomainPlainText;
            signature_html += "<br><br>" + sDomainHTML;
         }
         else if (sender_domain->GetSignatureMethod() == MA::Domain::SMOverwriteAccountSignature)
         {
            signature_plain_text = sDomainPlainText;
            signature_html = sDomainHTML;
         }            
