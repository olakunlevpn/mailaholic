...
typedef struct tagXMLEntitys_ : public std::vector<XENTITY>
{
	...
	MA::String Ref2Entity( LPCTSTR estr );
	MA::String Entity2Ref( LPCTSTR str );	
...
typedef struct _tagParseInfo
{
	...
	MA::String		error_string;		// [get] error string
...
typedef struct _tagXMLAttr
{
	...
	MA::String GetXML( LPDISP_OPT opt = &optDefault );
}XAttr, *LPXAttr;
...
typedef struct _tagXMLNode
{
	...
	MA::String GetXML( LPDISP_OPT opt = &optDefault );
	MA::String GetText( LPDISP_OPT opt = &optDefault );
   ...
   MA::String GetAttrValue( LPCTSTR attrname ); 
	XAttrs	GetAttrs( LPCTSTR name ); 

	// in one level child nodes
	LPXNode	GetChild( LPCTSTR name ); 
	LPCTSTR	GetChildValue( LPCTSTR name ); 
	MA::String	GetChildText( LPCTSTR name, LPDISP_OPT opt = &optDefault );
