...
{
   IMAPResult
   IMAPCommandCREATE::ExecuteCommand(std::shared_ptr<MA::IMAPConnection> pConnection, std::shared_ptr<IMAPCommandArgument> pArgument)
   {
   ...
   IMAPResult
   IMAPCommandCREATE::ConfirmPossibleToCreate(std::shared_ptr<MA::IMAPConnection> pConnection, const std::vector<String> &vecNewPath, bool bIsPublicFolder)
   {
      if (bIsPublicFolder)
      {
